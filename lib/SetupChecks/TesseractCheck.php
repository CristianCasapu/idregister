...
final class TesseractCheck implements ISetupCheck
{
    ...
    {
        return $this->l->t('IDRegister: text recognition');
    }
    ...
    public function run(): SetupResult
    {
        ...
        if (!$tesseract['ok']) {
            return SetupResult::error($this->l->t('No text recognition is installed, so documents cannot be read. Open Administration settings › IDRegister and press "Install the reader" (or run occ idregister:install-ocr).'));
        }

        return SetupResult::warning($this->l->t('Documents are read with Tesseract %s, which reads photographed cards poorly. Open Administration settings › IDRegister and press "Install the reader" (or run occ idregister:install-ocr).', [$tesseract['version']]));
    }
}
